...
async function runOfflineTransferCrossChain(
  node,
  networkId,
  chainId,
  targetChainId,
  ...
  senderPublicKey,
  senderPrivateKey,
  targetChainGasPayerAcct,
  targetChainGasPayerPublicKey,
  targetChainGasPayerPrivateKey
) {
  let proof;
  ...
  targetChainGasPayerAcct = await verifyTargetChainGasPayerOffline(
    targetChainGasPayerAcct
  );
  const receiverGuard = {
    pred: "keys-all",
  ...
  targetChainGasPayerPublicKey = await verifyTargetChainGasPayerPublicKeyOffline(
    targetChainGasPayerAcct,
    targetChainGasPayerPublicKey,
    targetChainId
  );
  targetChainGasPayerPrivateKey = await verifyTargetChainGasPayerPrivateKeyOffline(
    targetChainGasPayerAcct,
    targetChainGasPayerPrivateKey,
    targetChainId
  );
  ...
  const pactId = printCurlCmd(
    transferCrosschain.stepOne(
      senderAcct,
      senderPublicKey,
      senderPrivateKey,
      receiverAcct,
